...
(() => {
  ...
  function interact(){if(!state.playing)return;if(state.chapter===3){if(state.nearby==="communicator")collectCommunicator();else if(state.nearby==="coordinate")revealCoordinate();return;}if(state.twoD){showToast("靠近发光羽毛即可自动收集。",1300);return;}if(state.runner){showToast("航道会自动前进：切换跑道避障，按空格跳过低栏。",1800);return;}if(!state.nearby)return;if(state.chapter===2)captureFeather();else solveGarden();}
